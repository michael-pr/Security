...
def des(encrypt_or_decrypt, bv_input, result_bv, sboxs, rkeys):
	if(encrypt_or_decrypt == 'd'):
		rkeys = rkeys[::-1]											# Inverse the round keys if decryption
	else: pass
	...
	while (True):
		...
		if (len(bv_block) < 64):									# Catch non 8 byte multiples
			if (encrypt_or_decrypt == 'd'):
				...
				bv_block.pad_from_right(64 - len(bv_block))			# Pad block 
		else: pass
